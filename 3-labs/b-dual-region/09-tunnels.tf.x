...
locals {
  hub_eu_router_startup = templatefile("../../scripts/vyos/vyos.sh", {
    ...
    VPN_TUNNELS = [{
      ...
      local_vti_ip   = cidrhost(var.bgp_range.cidr1, 2)
      local_vti_mask = split("/", var.bgp_range.cidr1).1
      local_type     = "respond"
    ...
    BGP_SESSIONS = [
      {
        ...
        peer_ip          = cidrhost(var.bgp_range.cidr1, 1)
...
locals {
  site1_router_startup = templatefile("../../scripts/vyos/vyos.sh", {
    ...
    VPN_TUNNELS = [{
      ...
      local_vti_ip   = cidrhost(var.bgp_range.cidr1, 1)
      local_vti_mask = split("/", var.bgp_range.cidr1).1
      local_type     = "initiate"
    ...
    BGP_SESSIONS = [{
      ...
      peer_ip          = cidrhost(var.bgp_range.cidr1, 2)
      multihop         = { enable = true, ttl = 4 }
...
locals {
  hub_us_router_startup = templatefile("../../scripts/vyos/vyos.sh", {
    ...
    VPN_TUNNELS = [{
      ...
      local_vti_ip   = cidrhost(var.bgp_range.cidr2, 2)
      local_vti_mask = split("/", var.bgp_range.cidr2).1
      local_type     = "respond"
    ...
    BGP_SESSIONS = [
      {
        ...
        peer_ip          = cidrhost(var.bgp_range.cidr2, 1)
...
locals {
  site2_router_startup = templatefile("../../scripts/vyos/vyos.sh", {
    ...
    VPN_TUNNELS = [{
      ...
      local_vti_ip   = cidrhost(var.bgp_range.cidr2, 1)
      local_vti_mask = split("/", var.bgp_range.cidr2).1
      local_type     = "initiate"
    ...
    BGP_SESSIONS = [{
      ...
      peer_ip          = cidrhost(var.bgp_range.cidr2, 2)
      multihop         = { enable = true, ttl = 4 }
      route_map_export = { enable = true, map = "MAP-OUT-HUB" }
      route_map_import = { enable = true, map = "MAP-IN-HUB" }
    }]
