...
root_dir="$HOME/AWS/aws-network-terraform"
dest_dir="$HOME/TFSTATE/AWS"
...
script_path="$HOME/AWS/aws-network-terraform/helpers/scripts/tfstate.sh"
sudo bash -c "cat <<EOF > /etc/cron.d/tfstate-backup-aws
*/5 * * * * . $script_path 2>&1 > /dev/null
EOF"
crontab /etc/cron.d/tfstate-backup-aws
