...
module "spoke6_int_nlb" {
  ...
  listeners = [
    {
      name     = "${local.spoke6_prefix}int-nlb-fe-80"
      port     = 80
      protocol = "TCP"
      forward = {
        default      = true
        order        = 10
        target_group = "${local.spoke6_prefix}int-nlb-be-80"
      }
    },
    {
      name     = "${local.spoke6_prefix}int-nlb-fe-8080"
      port     = 8080
      protocol = "TCP"
      forward = {
        default      = true
        target_group = "${local.spoke6_prefix}int-nlb-be-8080"
      }
    }
  ]
  ...
  target_groups = [
    {
      name         = "${local.spoke6_prefix}int-nlb-be-80"
      protocol     = "TCP"
      port         = 80
    ...
    {
      name         = "${local.spoke6_prefix}int-nlb-be-8080"
      protocol     = "TCP"
      port         = 8080
      target       = { type = "instance", id = module.spoke6_vm.instance_id }
      vpc_id       = module.spoke6.vpc_id
...
resource "aws_vpc_endpoint" "spoke6_int_nlb_hub2" {
  ...
  tags = merge(local.spoke6_tags, {
    "Name" = "${local.spoke6_prefix}int-nlb-hub2"
  })
}
...
resource "aws_route53_record" "spoke6_int_nlb" {
  provider = aws.region2
  zone_id  = aws_route53_zone.region2.zone_id
  name     = local.spoke6_int_nlb_hostname
  type     = "A"
  ttl      = "60"
  records = [
